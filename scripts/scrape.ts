...
async function main() {
    ...
    for (const gadgetJSON of brawltimeNinjaGadgetJSON) {
        gadgetNameToIdDict[gadgetJSON["battle.gadgetName_measure"]] =
            gadgetJSON["battle.gadget_dimension"];
    }
    ...
    function getSkinURLFromName(links: any[], skinType: string, skinName: string) {
        ...
        return links.find(
            (link: string | any[]) => link.includes(brawlerFileName) && link.includes(skinFileName)
        );
    }

    function getAccessoryURLFromIndex(
        links: any[],
        kind: string,
        brawlerName: string,
        index: number
                ...
                for (const historyEntryElement of historySection.lists[0]) {
                    if (historyEntryElement.text.search("[0-9]+/[0-9]+/[0-9]+") != -1) {
                        ...
                        if (historyEntryDescriptions.length - 1 > historyEntryDescriptionCount) {
                            // @ts-ignore
                            historyEntryType =
                                historyEntryDescriptions[historyEntryDescriptionCount].list[0];
                        }
                        // @ts-ignore
